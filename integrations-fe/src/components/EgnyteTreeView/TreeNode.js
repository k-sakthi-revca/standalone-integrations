...
import { EgnyteContext } from './EgnyteContext';
...
const TreeNode = ({ node }) => {
  ...
  const { 
    ...
    setTargetNode, 
    moveFile 
  } = useContext(EgnyteContext);

  const isSelected = selectedNode && selectedNode.path === node.path;
  const isTarget = targetNode && targetNode.path === node.path;
  ...
  const handleSetTarget = (e) => {
    ...
    if (selectedNode && selectedNode.path === node.path) return; // Can't move to itself
    
    // Check if the selected node is a parent of this node (to prevent circular references)
    if (selectedNode && node.path.startsWith(selectedNode.path)) return; // Can't move to a child
  ...
  const handleMove = (e) => {
    ...
    if (selectedNode && targetNode) {
      // For Egnyte, we need the full paths
      const sourcePath = selectedNode.path;
      const destinationPath = targetNode.path + '/' + selectedNode.name;
